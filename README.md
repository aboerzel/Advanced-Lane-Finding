...
First, the chessboard corners of the calibration images are determined with OpenCV's `cv2.findChessboardCorners()` function. Afterwards the calibration coefficients are calculated with OpenCV's function `cv2.calibrateCamera()`.
After the calculation was done first time, the distortion coefficients will be stored in the pickle file `calibration.pkl`. 
On subsequent calls, the distortion coefficients are restored from the file.
